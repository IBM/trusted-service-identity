...
authenticate against this role`,
			...
			"metadata_claims": {
				Type:        framework.TypeCommaStringSlice,
				Description: `Comma-separated list of Claims to add as metadata to the identity`,
			},
		},
...
func (b *jwtAuthBackend) pathRoleCreateUpdate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	if metadataClaims, ok := data.GetOk("metadata_claims"); ok {
		role.MetadataClaims = metadataClaims.([]string)
	}
