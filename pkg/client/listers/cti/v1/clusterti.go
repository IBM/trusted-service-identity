/*
Copyright 2018 The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by lister-gen. DO NOT EDIT.

// This file was automatically generated by lister-gen

package v1

import (
	v1 "github.com/IBM/trusted-service-identity/pkg/apis/cti/v1"
	"k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/apimachinery/pkg/labels"
	"k8s.io/client-go/tools/cache"
)

// ClusterTILister helps list ClusterTIs.
type ClusterTILister interface {
	// List lists all ClusterTIs in the indexer.
	List(selector labels.Selector) (ret []*v1.ClusterTI, err error)
	// ClusterTIs returns an object that can list and get ClusterTIs.
	ClusterTIs(namespace string) ClusterTINamespaceLister
	ClusterTIListerExpansion
}

// clusterTILister implements the ClusterTILister interface.
type clusterTILister struct {
	indexer cache.Indexer
}

// NewClusterTILister returns a new ClusterTILister.
func NewClusterTILister(indexer cache.Indexer) ClusterTILister {
	return &clusterTILister{indexer: indexer}
}

// List lists all ClusterTIs in the indexer.
func (s *clusterTILister) List(selector labels.Selector) (ret []*v1.ClusterTI, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterTI))
	})
	return ret, err
}

// ClusterTIs returns an object that can list and get ClusterTIs.
func (s *clusterTILister) ClusterTIs(namespace string) ClusterTINamespaceLister {
	return clusterTINamespaceLister{indexer: s.indexer, namespace: namespace}
}

// ClusterTINamespaceLister helps list and get ClusterTIs.
type ClusterTINamespaceLister interface {
	// List lists all ClusterTIs in the indexer for a given namespace.
	List(selector labels.Selector) (ret []*v1.ClusterTI, err error)
	// Get retrieves the ClusterTI from the indexer for a given namespace and name.
	Get(name string) (*v1.ClusterTI, error)
	ClusterTINamespaceListerExpansion
}

// clusterTINamespaceLister implements the ClusterTINamespaceLister
// interface.
type clusterTINamespaceLister struct {
	indexer   cache.Indexer
	namespace string
}

// List lists all ClusterTIs in the indexer for a given namespace.
func (s clusterTINamespaceLister) List(selector labels.Selector) (ret []*v1.ClusterTI, err error) {
	err = cache.ListAllByNamespace(s.indexer, s.namespace, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterTI))
	})
	return ret, err
}

// Get retrieves the ClusterTI from the indexer for a given namespace and name.
func (s clusterTINamespaceLister) Get(name string) (*v1.ClusterTI, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusterti"), name)
	}
	return obj.(*v1.ClusterTI), nil
}
